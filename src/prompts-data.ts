...
3. **Trend Signal Analysis** — 200-300 words, distill from the current Trending snapshot and the last 7 days of topic-search activity:
...
2. **各维度热门项目** — 每个维度用 **Markdown 表格**呈现，列固定为：
   ...
   - **简要说明**：2 句话——项目是什么、当前为什么值得关注，点出关键数据或增长信号
   - 每个维度列 3~8 个项目；某维度下若无项目则整张表省略

3. **趋势信号分析** — 200~300 字，从当前 Trending 快照和过去7天主题搜索活动中提炼：
   - 哪类 AI 工具正在获得社区爆发性关注？
   - 有无新兴技术栈或方向首次登榜？
   - 与近期大模型发布/行业事件的关联
      ...
      const mode =
        lang === "en"
          ? isFirstRun
            ...
            : `Incremental update, ${newItems.length} new articles in the last 7 days`
          : isFirstRun
  ...
  const firstRunNote =
    lang === "en"
      ? isAnyFirstRun
        ...
        : "This is an incremental update. Please focus on the new content from the last 7 days and assess its strategic significance in context."
      : isAnyFirstRun
...
3. **Community Sentiment Signal** — 100-200 words analyzing the HN AI discussion mood and focus over the last 7 days:
...
3. **Market Signal** — 100-200 words analyzing Product Hunt AI launch patterns over the last 7 days:
...
export function buildArxivPrompt(data: ArxivData, dateStr: string, lang: Lang = "zh"): string {
  ...
  if (lang === "en") {
    return `You are an AI research analyst. The following are AI-related papers submitted to ArXiv in the last 7 days as of ${dateStr} (${data.papers.length} papers from cs.AI, cs.CL, cs.LG):
...
3. **Research Trend Signal** — 100-200 words on emerging research directions visible from submissions in the last 7 days
